...
class ChairAttributeDataset(Dataset):
    ...
    def _download_if_missing(self, row: Dict[str, Any], path: Path, force: bool = False) -> None:
        ...
        for url in urls:
            try:
                ...
                # Validate bytes are actually decodable image data before caching.
                ...
                return
            except (HTTPError, URLError, TimeoutError, OSError, UnidentifiedImageError) as exc:
                last_error = exc
                continue
        raise RuntimeError(
    ...
    def _load_image(self, row: Dict[str, Any]) -> Image.Image:
        ...
        try:
            ...
            # Retry once by re-downloading. Some VG image URLs occasionally produce
            # partial/bad files; this makes loading resilient instead of crashing workers.
            try:
...
def class_pos_weights(label_frequencies: Dict[str, int], label_vocab: Dict[str, int]) -> torch.Tensor:
...
def split_paths(processed_root: Path, split_dir_name: str = "splits") -> Dict[str, Path]:
